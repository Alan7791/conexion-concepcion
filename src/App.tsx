...
export default function App() {
  ...
  async function cargarDatos() {
    ...
    const { data } = await supabase
      .from('profesionales')
      .select('*')
      .order('es_verificado', { ascending: false }); // Prioridad a verificados
      ...
      <div className="min-h-screen bg-white p-6 flex flex-col items-center">
        ...
        <div className="w-full max-w-xl bg-slate-50/50 p-10 rounded-[3rem] border border-slate-100">
          ...
          <form onSubmit={async (e) => {
            ...
            <textarea placeholder="¿Qué servicios ofrecés?" className="p-5 bg-white border border-slate-200 rounded-2xl h-32 outline-none focus:ring-2 focus:ring-indigo-500 transition-all font-semibold" onChange={e => setNuevoPro({...nuevoPro, descripcion: e.target.value})} />
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FC] font-sans text-slate-900">
      <header className="bg-white pt-16 pb-12 px-6 border-b border-slate-100 text-center">
        <div className="max-w-5xl mx-auto">
          <div className="flex justify-center mb-8">
            <button onClick={() => { setBusqueda(""); window.scrollTo({top: 0, behavior: 'smooth'}); }} className="flex items-center justify-center bg-slate-900 w-16 h-16 rounded-2xl shadow-2xl transform hover:rotate-6 transition-all active:scale-90 group">
          ...
          <p className="text-slate-400 text-xl mb-12 font-medium">La red profesional más confiable de la Perla del Norte.</p>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {categoriasRapidas.map((cat, idx) => (
              <button key={idx} onClick={() => setBusqueda(cat.nombre)} className={`flex items-center gap-2 bg-white border border-slate-100 px-6 py-4 rounded-[1.3rem] shadow-sm hover:shadow-xl hover:text-indigo-600 transition-all ${busqueda === cat.nombre ? 'border-indigo-500 text-indigo-600' : ''}`}>
                <span className="text-slate-300 group-hover:text-indigo-500">{cat.icono}</span>
              ...
              <div key={p.id} className="group bg-white rounded-[2.5rem] p-8 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-slate-100 transition-all duration-500 hover:shadow-[0_30px_60px_-15px_rgba(79,70,229,0.15)] hover:-translate-y-2 flex flex-col relative overflow-hidden">
                ...
                <div className="relative mb-8 flex-grow">
                  <p className="text-slate-500 text-[14px] leading-relaxed font-medium italic pl-4 border-l-2 border-slate-100">
                    "{p.descripcion || `Profesional destacado en ${p.categoria} en Concepción.`}"
                  </p>
                </div>
                <div className="flex gap-3 mt-auto">
                  <a href={`https://wa.me/${p.whatsapp}`} target="_blank" className="flex-[3] bg-[#25D366] hover:bg-[#128C7E] text-white py-4 rounded-2xl font-black text-[11px] transition-all shadow-lg flex items-center justify-center gap-2 tracking-widest uppercase">
                ...
                {esAdmin && (
                  <div className="mt-6 pt-6 border-t border-slate-50 flex justify-between items-center">
                    <button onClick={() => toggleVerificado(p.id, p.es_verificado)} className="text-[9px] font-black p-2 bg-slate-100 rounded-lg">{p.es_verificado ? "DESVERIFICAR" : "VERIFICAR"}</button>
                    <button onClick={() => borrarProfesional(p.id)} className="text-red-300 hover:text-red-600"><Trash2 size={18}/></button>
                  </div>
                )}
              </div>
            ))}
